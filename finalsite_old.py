...
def get_content(web_page):
	...
	try:
		...
		if web_soup.find(class_='ptl_page').parent.find_all('form') != []:
			form = 'form'

		if web_soup.find(class_='ptl_page').parent.find_all('embed') != []:
			embed = 'embed'

		if web_soup.find(class_='ptl_page').parent.find_all('iframe') != []:
			iframe = 'iframe'

		if web_soup.find(class_='ptl_page').parent.find_all(class_='CAL-view') != []:
			calendar = 'calendar'

		if web_soup.find(class_='ptl_page').parent.find_all(class_='fsDirEntry') != []:
			staff = 'staff'

		if web_soup.find(class_='ptl_page').parent.find_all(class_='news-summary') != []:
			news = 'news'
		...
		# Content
		if web_soup.find(class_='ptl_page').parent != None and web_soup.find(class_='ptl_page').parent != '':
			col1 = web_soup.find(class_='ptl_page').parent
...
if __name__ == '__main__':
	...
	with open(f'../f_web_interface/static/files/{mainfolder}/report.csv', 'w', encoding='utf-8') as csv_report:
		...
		for site in all_sites:
			...
			sitemap = soup.find(class_='RadMenu')
			print(sitemap)
